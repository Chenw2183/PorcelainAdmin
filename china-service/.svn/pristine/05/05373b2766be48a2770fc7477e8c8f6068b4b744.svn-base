...
//        如果user_code== CmRole.role_code     user_code=""
...
//        System.out.println("~~~~~~~~~~~~~~~~~~" + cmRoleService.selectByRoleCode(pageQueryBean.getQueryCondition().getUserCode()).getRoleCode().equals(pageQueryBean.getQueryCondition().getUserCode()));

        if(cmRoleService.selectByRoleCode(pageQueryBean.getQueryCondition().getUserCode())!=null){
             cmPorcelain.setUserCode("");
        }



        if (cmPorcelain != null) {
            if (StringUtils.isNotEmpty(cmPorcelain.getUserCode())) {
                wrapper = wrapper.eq("user_code", cmPorcelain.getUserCode());
            }
